...
def load_sample_pd(sample, labels, data_root, label_type, threshold, imgTransform, use_s1, use_s2, use_RGB, IGBP_s):
    # load s2 data
    if use_s2:
        img = load_s2(os.path.join(data_root, sample.loc[(sample.index.get_level_values(0)[0], 's2'), 'path']),
                      imgTransform, s2_band=S2_BANDS_LD)
    # load only RGB
    if use_RGB and use_s2 == False:
        img = load_s2(os.path.join(data_root, sample.loc[(sample.index.get_level_values(0)[0], 's2'), 'path']),
                      imgTransform, s2_band=S2_BANDS_RGB)

    # load s1 data
    if use_s1:
        if use_s2 or use_RGB:
            img = np.concatenate((img, load_s1(os.path.join(data_root, sample.loc[(sample.index.get_level_values(0)[0], 's1'), 'path']),
                                               imgTransform)), axis=0)
        else:
            img = load_s1(os.path.join(data_root, sample.loc[(sample.index.get_level_values(0)[0], 's1'), 'path']), imgTransform)
...
class SEN12MS_selected(data.Dataset):
    """PyTorch dataset class for the SEN12MS dataset considering only a subset of the regions and tile
    """
    ...
    def __init__(self, ls_dir=None, imgTransform=None,
                 ...
                 use_s2=False, use_s1=False, use_RGB=False, IGBP_s=True, data_df: pd.DataFrame=None,
                 data_root: str=None):
        """Initialize the dataset"""

        # inizialize
        super(SEN12MS_selected, self).__init__()
        self.imgTransform = imgTransform
        self.threshold = threshold
        self.label_type = label_type
    ...
    def __getitem__(self, index):
        ...
        labels = self.labels
        return load_sample_pd(sample, labels, self.data_root, self.label_type, self.threshold, self.imgTransform,
                              self.use_s1, self.use_s2, self.use_RGB, self.IGBP_s)
